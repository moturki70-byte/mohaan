...
function render() {
    ...
    products.filter(p => p.cat == currentCat).forEach(p => {
        ...
        grid.innerHTML += `
            <div class="card">
                <h4>${p.name}</h4>
                <select id="size-${p.name}">${opts}</select>
                <div class="qty-box">
                    <button onclick="changeQty('${p.name}', -1)">−</button>
                    <span id="qty-${p.name}">1</span>
                    <button onclick="changeQty('${p.name}', 1)">+</button>
                </div>
                <button class="dalbo-btn" onclick="addToCart('${p.name}')">DALBO</button>
            </div>`;
    });
}
...
function addToCart(name) {
    ...
    alert(name + " dambiisha waa lagu daray!");
}
...
function renderCart() {
    ...
    cart.forEach((item, i) => {
        ...
        list.innerHTML += `<div class="cart-item" style="background:#fff; padding:10px; margin-bottom:5px; border-radius:10px;">
            <strong>${item.name}</strong> (${item.size}) x${item.qty} = ${sub}${item.price.c} 
            <button onclick="removeItem(${i})" style="float:right; color:red; border:none; background:none; cursor:pointer;">X</button>
        </div>`;
    });
...
function sendWhatsApp() {
    ...
    cart.forEach(c => {
        ...
        msg += `• ${c.name} (${c.size}) x${c.qty} = ${sub}${c.price.c}\n`;
    });
...
function showSalesReport() {
    document.getElementById("today-orders").innerText = dailySales.orders;
    document.getElementById("today-usd").innerText = "$" + dailySales.usd.toFixed(2);
    document.getElementById("today-sso").innerText = dailySales.sso + " SSO";
...
function editUserInfo() { localStorage.removeItem("macruuf_user"); location.reload(); }
function clearCart() {
    if(confirm("Ma rabtaa inaad kaydiso xisaabtan?")) {
        cart.forEach(item => {
            let sub = item.qty * item.price.p;
